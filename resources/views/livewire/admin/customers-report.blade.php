    ...
    <div class="grid gap-5 lg:grid-cols-2">
        @if ($canFinance)
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-5">
                ...
                @include('livewire.admin.partials.report-rank', ['rows' => $byOutstanding, 'value' => fn($r) => $fmt::usd($r['amount']), 'label' => fn($r) => $r['customer']?->name])
            </div>
            <div class="rounded-xl border border-slate-200 bg-white p-5">
                <h3 class="mb-3 font-semibold text-slate-800">الأعلى دفعات (USD)</h3>
                @include('livewire.admin.partials.report-rank', ['rows' => $byPayments, 'value' => fn($r) => $fmt::usd($r['amount']), 'label' => fn($r) => $r['customer']?->name])
            </div>
        @endif
        <div class="rounded-xl border border-slate-200 bg-white p-5">
