@props(['title' => null, 'pdf' => false, 'watermark' => null])
<!DOCTYPE html>
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .sheet {
            width: {{ $pdf ? '100%' : '210mm' }}; min-height: {{ $pdf ? 'auto' : '297mm' }};
            margin: {{ $pdf ? '0' : '12px auto' }}; padding: {{ $pdf ? '0' : '18mm' }};
            background: #fff; {{ $pdf ? '' : 'box-shadow: 0 1px 6px rgba(0,0,0,.08);' }}
            position: relative;
        }
        ...
        .draft-banner {
            margin: 0 0 14px; padding: 8px 14px; border: 2px dashed #b45309;
            border-radius: 8px; background: #fffbeb; color: #b45309;
...
<body>
    ...
    <div class="sheet">
        @if ($watermark)
            <div class="draft-banner">{{ $watermark }}</div>
        @endif
        {{ $slot }}
    </div>
</body>
